...
def create_map_coloring_csp() -> CSP:
    ...
    def are_neighbors(a, b) -> bool:
        return (a in neighbors and b in neighbors[a]) or \
            (b in neighbors and a in neighbors[b])

    # Adds all provinces as variables with domain |colors|
    for p in provinces:
        csp.add_variable(p, colors)
    # Adds binary factors to enforce that no two neighboring
    ...
    for p1 in provinces:
        for p2 in provinces:
            if are_neighbors(p1, p2):
                # In the lambda function, x and y are possible colors
                # from p1 and p2's domains respectively
                #
                # approach using lambda function
                # csp.add_binary_factor(p1, p2, lambda x, y : x != y)
                # approach using inner function
                csp.add_binary_factor(p1, p2, are_different)
    return csp
